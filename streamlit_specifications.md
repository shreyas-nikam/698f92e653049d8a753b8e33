...
# Streamlit Application Specification: Financial Model Validation and Performance Degradation Analysis

## 1. Application Overview

This Streamlit application provides a comprehensive framework for validating financial quantitative models, focusing on time-series cross-validation, regime-aware performance analysis, and performance degradation quantification. The app walks the user, personified by Sarah Chen (a CFA Charterholder and Portfolio Manager), through a rigorous validation workflow, demonstrating how to identify and mitigate common pitfalls in financial model deployment.

The application's high-level story flow mirrors Sarah Chen's step-by-step process of evaluating a new equity factor model. Starting with a fundamental demonstration of why traditional random splits fail for time-series data, it progresses through increasingly sophisticated cross-validation techniques (expanding window, sliding window, purged & embargo). It then stress-tests the model across various market regimes and diagnoses bias-variance trade-offs using learning curves. Finally, it synthesizes all findings into a "Model Validation Report," enabling a data-driven go/no-go deployment decision, adhering to CFA ethical standards and model risk management protocols.
...
from source.py import (
    generate_simulated_financial_data,
    evaluate_split_strategy,
    expanding_window_cv,
    sliding_window_cv,
    purged_embargo_walk_forward_cv,
    prepare_and_analyze_regime_performance, # Conceptual function, encapsulates regime analysis code block
    plot_learning_curve,
    generate_model_validation_report # Conceptual function, encapsulates final report code block
)
# Ensure warnings are suppressed as in source.py
import warnings
...
The application uses `st.session_state` to preserve data and calculation results across different user interactions and "pages."

**Initialization:**
Upon first run (`if 'data_generated' not in st.session_state:`):
...
*   `st.session_state.final_report_metrics = {}` # Dictionary to store all metrics for final report

**Update & Read:**
*   **"1. Random vs Temporal Split" page:**
    *   `generate_simulated_financial_data()` is called once to initialize `X`, `y`, `dates_aligned`, `model_config`. These are then stored in `st.session_state`.
    *   `evaluate_split_strategy()` is called with data from `st.session_state`. Results (`r2_random`, `r2_temporal`, `overfitting_gap`) are stored in `st.session_state`.
*   **"2. Expanding Window CV" page:**
    *   `expanding_window_cv()` is called with `st.session_state.X`, `st.session_state.y`, `st.session_state.dates_aligned`, `st.session_state.model_config`. Results (`expanding_results`, `expanding_mean_r2`, `expanding_std_r2`) are stored in `st.session_state`.
*   **"3. Sliding Window CV" page:**
    *   `sliding_window_cv()` is called with `st.session_state.X`, `st.session_state.y`, `st.session_state.dates_aligned`, `st.session_state.model_config`. Results (`sliding_results_df`, `sliding_mean_r2`, `sliding_std_r2`) are stored in `st.session_state`.
*   **"4. Purged & Embargo CV" page:**
    *   `purged_embargo_walk_forward_cv()` is called with `st.session_state.X`, `st.session_state.y`, `st.session_state.dates_aligned`, `st.session_state.model_config`, and crucially, `st.session_state.expanding_results['R2']` for leakage calculation. Results (`purged_scores`, `purged_mean_r2`, `purged_std_r2`, `leakage_inflation`) are stored in `st.session_state`.
*   **"5. Market Regime Analysis" page:**
    *   `prepare_and_analyze_regime_performance()` is called with `st.session_state.X`, `st.session_state.y`, `st.session_state.dates_aligned`, `st.session_state.model_config`. Results (`eval_df`, `regime_perf`) are stored in `st.session_state`.
*   **"6. Learning Curve" page:**
    *   `plot_learning_curve()` is called with `st.session_state.X`, `st.session_state.y`, `st.session_state.model_config`. Results (`learning_curve_diagnosis`) are stored in `st.session_state`.
*   **"7. Final Report" page:**
    *   All necessary metrics are read from `st.session_state` and assembled into `st.session_state.final_report_metrics`.
    *   `generate_model_validation_report()` is called with `st.session_state.final_report_metrics` and other individual metrics.

### Application Structure and Flow

The application simulates a multi-page experience using a Streamlit sidebar `selectbox`.

```python
st.set_page_config(layout="wide", page_title="Financial Model Validation")
...
if page == "Introduction":
    ...
    if st.button("Generate Simulated Financial Data"):
        X, y, dates_aligned, model_config = generate_simulated_financial_data()
    ...
    if st.session_state.data_generated:
        ...
        if st.button("Run Sliding Window CV"):
            # The sliding_window_cv function in source.py directly creates and shows a matplotlib plot.
            # We need to capture this plot for Streamlit. The function internally uses plt.show(),
            # which might block or not display correctly in Streamlit.
            # Assuming the source.py function can be modified to return the figure object
            # or that plt.show() is effectively replaced with a no-op / rendering to a buffer.
            # For this spec, we assume it generates a figure that `st.pyplot` can display.
            fig_sliding_cv = plt.figure() # Placeholder for plot generated by function
            ...
            st.pyplot(fig_sliding_cv) # Display plot generated by the function
            ...
            # Re-generate the plot if needed for display on revisiting the page
            # Or assume the function itself can recreate without re-running CV.
            # For blueprint, let's assume it can be displayed if data exists.
    ...
    if st.session_state.data_generated:
        if st.button("Run Market Regime Analysis"):
            # This conceptual function handles fetching market data, regime classification,
            # model fitting on full data, prediction, and regime-conditional performance calculation and plotting.
            eval_df, regime_perf, fig1, fig2 = prepare_and_analyze_regime_performance(
                st.session_state.X, st.session_state.y, st.session_state.dates_aligned, st.session_state.model_config
            )
            ...
            st.pyplot(fig1) # Box plot of residuals
            st.pyplot(fig2) # Bar chart of R2 by regime
            ...
            # Assuming prepare_and_analyze_regime_performance can return figures without re-computation
            _, _, fig1_re, fig2_re = prepare_and_analyze_regime_performance(
                st.session_state.X, st.session_state.y, st.session_state.dates_aligned, st.session_state.model_config,
    ...
    if st.session_state.data_generated:
        if st.button("Run Learning Curve Analysis"):
            from sklearn.model_selection import TimeSeriesSplit # Ensure TimeSeriesSplit is available
            tscv_lc = TimeSeriesSplit(n_splits=5)
            # plot_learning_curve also generates a plot
            ...
            # Re-generate plot for display if needed
    ...
    if st.session_state.data_generated and st.session_state.r2_random is not None and st.session_state.purged_mean_r2 is not None and st.session_state.regime_perf is not None and st.session_state.learning_curve_diagnosis is not None:
        if st.button("Generate Model Validation Report"):
            # Assemble all metrics into a dictionary for the report generation function
            report_metrics = {
            ...
            st.session_state.final_report_metrics = report_metrics # Store for re-display
